...
if __name__ == '__main__':
    ...
    im = cv2.imread("ir1.png",1)
    im2 = cv2.imread("vi1.png",1)
    #im = cv2.resize(im, (620,440))
    im = np.expand_dims(im, axis=0)
    im2 = np.expand_dims(im2, axis=0)
    print(im.shape)
    im = np.concatenate([im]*5, axis=0)
    im2 = np.concatenate([im2]*5, axis=0)
    print(im.shape)
    ...
    coeff = batch_multi_channel_pdfbdec(x=im, pfilt="9/7", dfilt="qmf2", nlevs=[3])
    xlo2,xhi2 = ContourDec(nlevs)(im2)
    print(len(coeff[0]))
    print(len(coeff[1]))
    print(torch.max(xlo))
    print(torch.min(xlo))
    print(xhi[0].shape)
    print(torch.max(xhi[0]))
    print(torch.min(xhi[0]))
    print(xhi[1].shape)
    print(xhi[2].shape)
    print(xhi[3].shape)
    print(torch.max(xhi[5]))
    print(torch.min(xhi[5]))
    print(xhi[4].shape)
    print(xhi[5].shape)
    print(xhi[6].shape)
    print(xhi[7].shape)
